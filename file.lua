...
if not game:IsLoaded() then
	...
	notLoaded.Text = "Infly-Admin is waiting for the game to load"
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Infly-Admin (" .. currentVersion .. ")"
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infly-Admin Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'crash / lag / stopserver', DESC = 'Kill the game your on.'}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Infly-Admin support server.'}
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/HelloIlovegimkit4/infly-admin-with-dex-re/refs/heads/main/file.lua'))()")
	end
end)
...
--$save1 / crash / lag / stopserver
addcmd('crash', {'lag', 'stopserver'}, function(args, speaker)
	--crash the server apparently but more chaotic and faster, also will spin u alot
	--spinnn before crash
	local spinSpeed = 5
	...
	for i = 1, 10000 do --repeat 10000 times
		task.spawn(crash)--crash all 1th time
		task.spawn(crash)--crash all 2th time
		task.spawn(crash)--crash all 3th time
		task.spawn(crash)--crash all 4th time
		task.spawn(crash)--crash all 5th time
		task.spawn(crash)--crash all 6th time
		task.spawn(crash)--crash all 7th time
		task.spawn(crash)--crash all 8th time
		task.spawn(crash)--crash all 9th time
		task.spawn(crash)--crash all 10th time
	end
end)
...
task.spawn(function()
	wait()
	notify("secret", "this is actually the private version")
	Credits:TweenPosition(UDim2.new(0, 0, 0.9, 0), "Out", "Quart", 0.2)
	Logo:TweenSizeAndPosition(UDim2.new(0, 175, 0, 175), UDim2.new(0, 37, 0, 45), "Out", "Quart", 0.3)
	Logo.ImageTransparency = 1
	IntroBackground.BackgroundTransparency = 1
	wait(1)
	local OutInfo = TweenInfo.new(0, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, false, 0)
